...
yearly_aggregated_data = df.groupby('Year').agg({
    'Avg_Housing_Value': 'mean',
    'Avg_Total_CPI': 'mean',
    'Avg_Bank_Interest_Rate_Weekly': 'mean'
}).reset_index()

# Function to generate response based on yearly aggregated data
def generate_response_with_prompt_engineering(aggregated_data, max_tokens=100, temperature=0.4, top_p=0.9):
    engineered_prompt = (
        f"Based on the following aggregated data for Canada, analyze the trends and interactions between the housing market, "
        f"CPI, and mortgage interest rates for each year: "
        f"Year: {aggregated_data['Year']}, "
        f"Average CPI: {aggregated_data['Avg_Total_CPI']}, "
        f"Average bank interest rate: {aggregated_data['Avg_Bank_Interest_Rate_Weekly']}, "
        f"Average housing price: {aggregated_data['Avg_Housing_Value']}. "
        "Please focus on understanding and anlysing the relationship and trends between these metrics over the years and provide explaiantion."
    )
    ...
    inputs = tokenizer(engineered_prompt, return_tensors="pt", truncation=True, max_length=512)
    ...
    return engineered_prompt, generated_response

# Generate the response based on aggregated data for each year
engineered_prompt, response = generate_response_with_prompt_engineering(yearly_aggregated_data, max_tokens=400, temperature=0.4)
...
app.layout = html.Div([
  ...
  html.H1("Trend Analysis of Housing Prices with CPI, Bank Interest Rates, and Housing Market Metrics", style={'text-align': 'center'}),

    # Display the response generated from the model
    html.Div([
        html.H3("LLM Generated Response:"),
        html.P(f"{response}")
    ], style={'margin': '20px'}),

    # Bar plot for yearly trend analysis of housing prices, CPI, and interest rates
    dcc.Graph(
        id='yearly-trend-analysis-barplot',
        figure={
            'data': [
                {'x': yearly_aggregated_data['Year'], 'y': yearly_aggregated_data['Avg_Housing_Value'], 'type': 'bar', 'name': 'Housing Prices'},
                {'x': yearly_aggregated_data['Year'], 'y': yearly_aggregated_data['Avg_Total_CPI'], 'type': 'bar', 'name': 'CPI Index'},
                {'x': yearly_aggregated_data['Year'], 'y': yearly_aggregated_data['Avg_Bank_Interest_Rate_Weekly'], 'type': 'bar', 'name': 'Bank Interest Rates'},
            ],
            'layout': {
                'title': 'Yearly Trends of Housing Prices, CPI, and Bank Interest Rates',
                'xaxis': {'title': 'Year'},
                'yaxis': {'title': 'Values'},
                'barmode': 'group',  # Group the bars for different metrics for each year
                'showlegend': True
            }
        }
    )
